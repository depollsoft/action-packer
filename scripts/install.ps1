...
if (Test-Path $RootEnvFile) {
    ...
    Write-StatusMessage "Using existing environment file at $BackendEnvFile"
    Write-InfoMessage "Your existing configuration will be preserved."
} else {
    # No existing .env found, create one in backend folder
    $EnvContent = @"
...
"@
    ...
    Write-StatusMessage "Environment template created at $BackendEnvFile"
    Write-WarningMessage "Please edit $BackendEnvFile to configure your GitHub App settings"
}
